...
| M3  | **Shared Auth + Legal once** after selection      | Auth is isolated; Prepaid/B2B2C run auth _after_ flow-specific prefix in config         | **Critical** | Prepaid, B2B2C |
...
| **Prepaid activation**  | `prepaid.entry` → `prepaid.activation-code` → `prepaid.code-validation`  | PR01–PR03 _(move after auth)_ |
...
| ---------- | --------------------------------------- | ---------------- | ------------------------------------------------------------ |
| `purchase` | Consumer QR Activation + Purchase       | `purchase`       | `purchase.qr-scan` _(recommended)_ or `purchase.plan-select` |
...
| -------------- | ---------------------- | ------------------------- |
| `purchase`     | `purchase.qr-scan`     | QrScan _(new)_ → then P01 |
...
**Alternative (MVP):** Skip QrScan; treat P01 as activation entry — _document as product debt_.
