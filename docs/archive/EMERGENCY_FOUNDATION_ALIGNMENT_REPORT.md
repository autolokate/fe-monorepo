...
`purchase-plans.ts` Secure feature line updated from _"3 contacts + AI calling"_ → _"2 contacts + AI calling"_ to match limits.
...
R0 always shows _"1 rider's cover"_ regardless of `riderCount === 2`. Foundation only skips R0 when entitlement is zero.
