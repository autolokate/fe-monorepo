...
export function setOnboardingAuthFailureHandler(handler: (() => void) | null): void {
  authFailureHandler = handler;
}

/** Public / bootstrap client — OTP, refresh, legal docs. No auth retry loop. */
export function getOnboardingBootstrapClient(): ApiClient {
  if (!bootstrapClient) {
...
export function getOnboardingApiClient(): ApiClient {
  if (!authenticatedClient) {
    const tokenManager = getTokenManager();
    wireTokenRefresh(tokenManager, getOnboardingBootstrapClient());
