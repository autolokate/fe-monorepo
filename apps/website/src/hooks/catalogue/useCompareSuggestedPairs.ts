...
export function useCompareSuggestedPairs(vehicleCategory: VehicleCategory) {
  const query = useApiQuery(() => fetchCompareSuggestedEntries(vehicleCategory), [vehicleCategory]);

  const pairs = useMemo(() => groupCompareSuggestedPairs(query.data ?? []), [query.data]);
