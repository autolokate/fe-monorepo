...
function renderChipState(state: ShowcaseState) {
  ...
  return (
    <AlChip variant={variant} label="Active" icon={<AlIcon name="user" size={14} aria-hidden />} />
  );
}
...
export function AlChipPage() {
  return (
    <CoreComponentShowcase
      ...
      overview={
        <p>Read-only status chip with colored dot and label. Variants map to Figma exactly.</p>
      }
      variants={
        <div className="preview-row">
          ...
          <AlChip
            variant="green"
            label="With icon"
            icon={<AlIcon name="shield-check" size={14} aria-hidden />}
          />
        </div>
      }
...
<AlChip variant="neutral" label="Label" />`}
      props={[
        {
          name: 'variant',
          type: "'green' | 'amber' | 'neutral'",
          defaultValue: 'neutral',
          description: 'Figma chip color variant.',
        },
